...
class utils:
    ...
    def step(self,s, a, h):
        ...
        next_state,obj_c,con_c = int(np.random.choice(np.arange(self.N_STATES),1,replace=True,p=probs)),self.R[s][a],self.C[s][a]
    ...
    def compute_lang_opt(self,k):
        ...
        eps_C = 1 * (self.EPISODE_LENGTH**1.5) * np.sqrt(((self.N_STATES)**2)*self.N_ACTIONS)
        ...
        for s in range(self.N_STATES):
            R[s] = {}
            for a in self.ACTIONS[s]:
                R[s][a] = self.R[s][a] - (self.lmbda/nu)*self.C[s][a]
    ...
    def compute_lang_opt1(self,k,NUMBER_EPISODES):
        ...
        eps_C = 0.062 * (self.EPISODE_LENGTH**2) * np.sqrt(((self.N_STATES)**3)*self.N_ACTIONS)
        ...
        for s in range(self.N_STATES):
            ...
            totR[s] = {}
            for a in self.ACTIONS[s]:
                beta[s][a] = np.sqrt(L/max(self.NUMBER_OF_OCCURANCES[s][a],1 ))
                newC[s][a] = self.C[s][a] - (1 + self.EPISODE_LENGTH*self.N_STATES)*beta[s][a]
                totR[s][a] = self.R[s][a] + (1 + self.EPISODE_LENGTH*self.N_STATES)*beta[s][a] - (self.lmbda/nu)*newC[s][a]
    ...
    def valueiteration(self,P,R):
        ...
        for i in range(self.EPISODE_LENGTH):
            ...
            for s in range(self.N_STATES):
                ...
                for a in self.ACTIONS[s]:
                    qVals[j,s][a] = R[s][a] + np.dot(P[s][a], qMax[j + 1])

                opt_policy[j][s] = int(np.argmax(qVals[j,s]))
                qMax[j][s] = np.max(qVals[j,s])
